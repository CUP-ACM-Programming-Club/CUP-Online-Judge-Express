...
describe("test ban_check", function () {
    ...
    it('should return when next is not a function', function (done) {
        ban_check({session: {auth: true, isadmin: true}}, {});
        done();
    });
    ...
    it('should be banned', function (done) {
        ban_check({
            ...
            json: function (data) {
                expect(data).to.have.property("status").that.equal("error");
                expect(data).to.have.property("statement");
                expect(data.statement).to.have.property("ban").that.equal(true);
                done();
            }
        }, done);
    });

    it('should allow when ban expired', function (done) {
        ban_check({
            session: {
            ...
            json: function () {
                done(new Error("should not be banned"));
            }
        }, done);
    });
