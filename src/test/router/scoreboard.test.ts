...
describe("test scoreboard", function () {
    ...
    before(async function () {
        ...
        await query(`insert into source_code_user(solution_id, source, hash)values(?,?,?)`,
            [1000, "console.log('hello user')", "hash"]);
    ...
    it("scoreboard should receive a JSON object", function (done) {
    ...
    it('scoredetail should receive a JSON object and merged map', function (done) {
        request.get("/scoreboard/1000/line")
            .expect(200)
            .end(function (err, res) {
                if (err) return done(err);
                expect(res.body).to.have.ownProperty("status").that.equal("OK");
                ...
                // Verify map has the solution and merged line info
                // line info comes from lineBreakHandler which queries source_code_user length
                if (map["1000"]) {
                    expect(map["1000"]).to.have.property("line");
                }
                done();
            })
    });

    it("should reject invalid contest id", function (done) {
        request.get("/scoreboard/abc")
            .expect(200)
            .end(function (err, res) {
    ...
    it("should reject contest id below 1000", function (done) {
        request.get("/scoreboard/999")
            .expect(200)
            .end(function (err, res) {
                expect(res.body).to.have.property("status").that.equal("error");
                done();
