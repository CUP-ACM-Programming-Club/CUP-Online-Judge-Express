...
describe("test export", function () {
    ...
    before(async function () {
        ...
        fakeDb.registerHandler(
            (sql) => sql.toLowerCase().includes("from source_code right join"),
            () => ([
                {user_id: "test", nick: "test", problem_id: 1000, result: 4, source: "code", language: 1, solution_id: 1000},
                {user_id: "test", nick: null, problem_id: 1001, result: 4, source: "code2", language: 1, solution_id: 1001}
            ])
        );
