...
router.get("/:user_id", async (req, res) => {
	...
	let sqlQueue = [];
	sqlQueue.push(query(`
select * from (select "LOCAL" as oj_name,problem_id,result,language from solution where user_id = ? and problem_id > 0
union all
select oj_name,problem_id,result,language from vjudge_solution where user_id = ?
union all
select oj_name,problem_id,result,language from vjudge_record where user_id = ? and oj_name != "CUP"
)t
`, [user_id, user_id, user_id]));
	...
	sqlQueue.push(query("select avatar,school,email,blog,github,nick from users where user_id = ?", [user_id]));
